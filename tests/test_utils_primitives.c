...
void run_tests_utils_primitives(void)
{
    ...
    TEST_GROUP("max_of_array / min_of_array / min_of_array_index");
    {
        ...
        ASSERT(min_of_array_index(arr, 8) == 1, "min_of_array_index: first min at index 1");

        int single[] = {42};
        ASSERT(max_of_array(single, 1) == 42, "max_of_array(single): 42");
        ASSERT(min_of_array(single, 1) == 42, "min_of_array(single): 42");
        ASSERT(min_of_array_index(single, 1) == 0, "min_of_array_index(single): index 0");

        int neg[] = {-5, -1, -3, -2};
        ASSERT(min_of_array(neg, 4) == -5, "min_of_array: negatives, min is -5");
        ASSERT(max_of_array(neg, 4) == -1, "max_of_array: negatives, max is -1");
    }
    ...
    TEST_GROUP("free_array_char");
    {
        ...
        char **ss = (char **)xmalloc(sizeof(char *) * 3);
