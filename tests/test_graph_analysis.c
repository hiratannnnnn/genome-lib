...
void run_tests_graph_analysis(void)
{
    TEST_GROUP("compute_degrees");
    {
        ...
        ASSERT(degree[0] == 1, "compute_degrees: vertex 0 degree = 1 (path graph)");
        ASSERT(degree[1] == 2, "compute_degrees: vertex 1 degree = 2 (path graph)");
        ASSERT(degree[2] == 1, "compute_degrees: vertex 2 degree = 1 (path graph)");
    ...
    TEST_GROUP("is_tree");
    {
        ...
        int **disc = gen_matrix_int(4, 4);
        disc[0][1] = disc[1][0] = 1;
        disc[2][3] = disc[3][2] = 1;
        ASSERT(is_tree(disc, 4) == 0, "is_tree: disconnected graph (not a tree)");
        free_matrix_int(disc, 4, 4);

        /* Single vertex: is a tree */
        int **single = gen_matrix_int(1, 1);
    ...
    TEST_GROUP("find_odd_vertices");
    {
        ...
        int cnt = find_odd_vertices(deg_path, 3, odd_list);
        ASSERT(cnt == 2, "find_odd_vertices: path graph has 2 odd-degree vertices");
        ...
        int deg_k3[] = {2, 2, 2};
        int odd_k3[3] = {-1, -1, -1};
        int cnt2 = find_odd_vertices(deg_k3, 3, odd_k3);
        ...
        int odd_star[5];
        int cnt3 = find_odd_vertices(deg_star, 5, odd_star);
        ASSERT(cnt3 == 4, "find_odd_vertices: star graph has 4 odd-degree vertices (leaves)");
    }

    TEST_GROUP("is_undigraph");
    {
        /* Symmetric matrix -> undirected */
        int **sym = gen_matrix_int(3, 3);
        sym[0][1] = sym[1][0] = 1;
        sym[1][2] = sym[2][1] = 1;
        ...
        ASSERT(is_undigraph(sym, 3) == 0, "is_undigraph: asymmetric matrix = 0");
        free_matrix_int(sym, 3, 3);

        /* Zero matrix is trivially symmetric */
        int **zero = gen_matrix_int(4, 4);
    ...
    TEST_GROUP("compute_degrees_from_list");
    {
        ...
        ASSERT(degree[0] == 1, "compute_degrees_from_list: vertex 0 degree = 1");
        ASSERT(degree[1] == 2, "compute_degrees_from_list: vertex 1 degree = 2");
        ASSERT(degree[2] == 1, "compute_degrees_from_list: vertex 2 degree = 1");
