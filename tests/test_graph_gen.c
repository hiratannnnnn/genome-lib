...
void run_tests_graph_gen(void)
{
    ...
    {
        int **m = gen_rand_undigraph(6, 0.5);
        ...
        int **m1 = gen_rand_undigraph(4, 1.0);
        int all_edges = 1;
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                if (i != j && m1[i][j] != 1) all_edges = 0;
    ...
    {
        int **m = gen_rand_conn_undigraph(6, 0.3);
        ...
        int **m2 = gen_rand_conn_undigraph(2, 0.0);
        ASSERT(m2[0][1] == 1 && m2[1][0] == 1, "gen_rand_conn_undigraph(n=2): edge always present");
        free_matrix_int(m2, 2, 2);
    }

    TEST_GROUP("gen_rand_digraph");
    {
        int **m = gen_rand_digraph(5, 0.5);
    ...
    TEST_GROUP("gen_rand_DAG");
    {
         ...
         * We check: for each edge i->j, verify topological ordering is maintained.
         * gen_rand_DAG uses a random permutation (topnr), and only adds edges
        ...
        for (int i = 0; i < 6; i++)
            for (int j = 0; j < 6; j++)
                if (m[i][j] == 1 && m[j][i] == 1) no_mutual = 0;
    ...
    TEST_GROUP("gen_rand_bigraph");
    {
         ...
         */
        int n = 4;
        int **m = gen_rand_bigraph(n, n, 0.7);
        ...
        /* prob=0 -> no edges */
        int **m0 = gen_rand_bigraph(n, n, 0.0);
    ...
    TEST_GROUP("gen_rand_undigraph_intcost");
    {
         ...
         */
        int **m = gen_rand_undigraph_intcost(5, 1, 10, 0.6);
        ASSERT(m != NULL, "gen_rand_undigraph_intcost: non-NULL");
        ASSERT(is_symmetric(m, 5), "gen_rand_undigraph_intcost: symmetric");
        int valid = 1;
        for (int i = 0; i < 5; i++)
            for (int j = 0; j < 5; j++)
                if (m[i][j] != INT_MAX / 2 && m[i][j] != 0
                    && (m[i][j] < 1 || m[i][j] > 10))
                    valid = 0;
        ASSERT(valid, "gen_rand_undigraph_intcost: costs in [1,10] or INT_MAX/2 (no-edge)");
        free_matrix_int(m, 5, 5);
    }

    TEST_GROUP("gen_rand_digraph_intcost");
    {
        int **m = gen_rand_digraph_intcost(5, 2, 8, 0.5);
        ASSERT(m != NULL, "gen_rand_digraph_intcost: non-NULL");
        /* diagonal is 0 (skipped by implementation), off-diag: cost or INT_MAX/2 */
        ASSERT(m[0][0] == 0, "gen_rand_digraph_intcost: diagonal is 0");
        int valid = 1;
        for (int i = 0; i < 5; i++)
            for (int j = 0; j < 5; j++)
                if (i != j && m[i][j] != INT_MAX / 2
                    && (m[i][j] < 2 || m[i][j] > 8))
                    valid = 0;
        ASSERT(valid, "gen_rand_digraph_intcost: costs in [2,8] or INT_MAX/2 (no-edge)");
        free_matrix_int(m, 5, 5);
    }

    TEST_GROUP("gen_rand_undigraph_dbcost");
    {
        double **m = gen_rand_undigraph_dbcost(4, 0.5, 5.0, 0.6);
        ASSERT(m != NULL, "gen_rand_undigraph_dbcost: non-NULL");
        /* Symmetric: cost[i][j] == cost[j][i] */
        int sym = 1;
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                if (m[i][j] != m[j][i]) sym = 0;
