...
void run_tests_graph_conversion(void)
{
    TEST_GROUP("adj_matrix_to_vertices (undirected)");
    {
         ...
         */
        int **m = gen_matrix_int(3, 3);
        m[0][1] = m[1][0] = 1;
        m[1][2] = m[2][1] = 1;

        Vertex **vs = adj_matrix_to_vertices(m, 3, 1);
        ASSERT(vs != NULL, "adj_matrix_to_vertices (undir): non-NULL");
        ASSERT(len_edge_list(vs[0]->incidence) == 1, "adj_matrix_to_vertices (undir): vs[0] has 1 edge");
        ASSERT(len_edge_list(vs[1]->incidence) == 2, "adj_matrix_to_vertices (undir): vs[1] has 2 edges");
        ASSERT(len_edge_list(vs[2]->incidence) == 1, "adj_matrix_to_vertices (undir): vs[2] has 1 edge");
    ...
    TEST_GROUP("adj_matrix_to_vertices (directed)");
    {
        ...
        Vertex **vs = adj_matrix_to_vertices(m, 3, 0);
        ASSERT(vs != NULL, "adj_matrix_to_vertices (dir): non-NULL");
        ASSERT(len_edge_list(vs[0]->incidence) == 1, "adj_matrix_to_vertices (dir): vs[0] has 1 edge");
        ASSERT(len_edge_list(vs[1]->incidence) == 1, "adj_matrix_to_vertices (dir): vs[1] has 1 edge");
        ASSERT(len_edge_list(vs[2]->incidence) == 0, "adj_matrix_to_vertices (dir): vs[2] has 0 edges");
        free_vertex_array(vs, 3);
        free_matrix_int(m, 3, 3);
    }
    ...
    TEST_GROUP("vertices_to_adj_matrix");
    {
        ...
        int **m = vertices_to_adj_matrix(vs, 3);
        ASSERT(m != NULL, "vertices_to_adj_matrix: non-NULL");
        ASSERT(m[0][1] == 1 && m[1][0] == 1, "vertices_to_adj_matrix: edge 0-1");
        ASSERT(m[1][2] == 1 && m[2][1] == 1, "vertices_to_adj_matrix: edge 1-2");
        ASSERT(m[0][2] == 0 && m[2][0] == 0, "vertices_to_adj_matrix: no edge 0-2");
        ASSERT(m[0][0] == 0 && m[1][1] == 0, "vertices_to_adj_matrix: no self-loops");
        free_matrix_int(m, 3, 3);
        free_vertex_array(vs, 3);
    }
    ...
    TEST_GROUP("vertices_to_adj_matrix roundtrip");
    {
        ...
        int **orig = gen_matrix_int(4, 4);
        orig[0][1] = orig[1][0] = 1;
        orig[0][3] = orig[3][0] = 1;
        orig[2][3] = orig[3][2] = 1;

        Vertex **vs = adj_matrix_to_vertices(orig, 4, 1);
        int **recovered = vertices_to_adj_matrix(vs, 4);

        int ok = 1;
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                if (orig[i][j] != recovered[i][j]) ok = 0;
    ...
    TEST_GROUP("comp_adj_matrix");
    {
         ...
         */
        int **empty = gen_matrix_int(3, 3);
        int **comp_empty = comp_adj_matrix(empty, 3);
        ...
        int **k3 = gen_matrix_int(3, 3);
        k3[0][1] = k3[1][0] = k3[0][2] = k3[2][0] = k3[1][2] = k3[2][1] = 1;
        int **comp_k3 = comp_adj_matrix(k3, 3);
    ...
    TEST_GROUP("comp_adj_list");
    {
        ...
        Vertex **comp = comp_adj_list(m, 3, 1);
    ...
    TEST_GROUP("cost_matrix_to_vertices");
    {
        ...
        double **cm = gen_matrix_double(3, 3);
        cm[0][1] = cm[1][0] = 2.5;
        cm[0][2] = cm[2][0] = DBL_MAX / 2; /* no edge */
        cm[1][2] = cm[2][1] = 1.5;

        Vertex **vs = cost_matrix_to_vertices(cm, 3, 1);
        ...
        Edge *e01 = get_target_edge(vs[0]->incidence, 1);
    ...
    TEST_GROUP("dbcost_to_adj");
    {
         ...
         */
        int **cost = gen_matrix_int(3, 3);
        cost[0][1] = cost[1][0] = 5;
        cost[1][2] = cost[2][1] = 0; /* no edge */

        int **adj = dbcost_to_adj(cost, 3);
