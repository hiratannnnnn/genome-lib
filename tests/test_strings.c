...
void	run_tests_strings(void)
{
	...
	TEST_GROUP("ft_split");
	{
		...
		for (int i = 0; res2[i]; i++)
			xfree(res2[i], strlen(res2[i]) + 1);
	...
	TEST_GROUP("ft_strtrim");
	{
		...
		r2 = ft_strtrim("--hello-world--", "-");
		ASSERT(strcmp(r2, "hello-world") == 0,
			"ft_strtrim: trims dashes both sides");
		free(r2);
		/* No trimming needed */
		r3 = ft_strtrim("hello", " ");
		...
		ASSERT(strlen(r4) == 0,
			"ft_strtrim: all-trim-chars yields empty string");
		free(r4);
		/* Empty string */
		r5 = ft_strtrim("", " ");
		...
		r8 = ft_strtrim("xyzABCxyz", "xyz");
		ASSERT(strcmp(r8, "ABC") == 0,
			"ft_strtrim: trims multiple chars from set");
		free(r8);
	}
}
