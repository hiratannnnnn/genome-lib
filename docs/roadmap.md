...
### コア機能構想：制約ベースのアルゴリズム自動選択

ゲノム再配列問題は「染色体数・signed/unsigned・linear/circular・indel有無・重複遺伝子の有無」などの制約の組み合わせによって、使うべきアルゴリズムが大きく変わる。毎回一から書き直すのが手間なので、**制約を渡すと適切なアルゴリズム（関数）を返す仕組み**を C の関数として実装することを目標とする。
...
制約パターンの詳細は `docs/patterns.md` を参照。
